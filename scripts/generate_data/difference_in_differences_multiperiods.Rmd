...
ここでは、第10章の10.2節「異なる時点に処置割当が生じる場合の分解・集計手法」で解説した差の差推定の方法を、`did`パッケージを用いて実装する。`TestAI`から呼び出している関数は`R/functions_did.R`に定義されている。
...
```{r}
# 出力ディレクトリの作成
output_dir <- "output/difference_in_differences_multiperiods"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}

# データセットの保存
saveRDS(df_design_nocov, file.path(output_dir, "df_design_nocov.rds"))
saveRDS(df_design_cov, file.path(output_dir, "df_design_cov.rds"))
saveRDS(df_design_nyt, file.path(output_dir, "df_design_nyt.rds"))

# CSV形式でも保存
write.csv(df_design_nocov, file.path(output_dir, "df_design_nocov.csv"), row.names = FALSE)
write.csv(df_design_cov, file.path(output_dir, "df_design_cov.csv"), row.names = FALSE)
write.csv(df_design_nyt, file.path(output_dir, "df_design_nyt.csv"), row.names = FALSE)

cat("データセットを", output_dir, "に保存しました。\n")
```
