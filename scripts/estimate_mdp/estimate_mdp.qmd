...
# Load solver configuration for shared parameters
...
# Create beta grid for search
beta_grid = np.linspace(0.5, 1.5, 11)  # Search around true value of 1.0
...
# Run estimation
print(f"\nRunning estimation...")
gamma_hat, beta_hat = EstimateMDP(
    states=states,
    actions=actions,
    delta=delta,
    N=N,
...
solver_dir = Path('../../output/sovle_mdp')
...
v_theta_0_est, v_theta_1_est = SolveValueIteration(
    beta=beta_hat,
    gamma=gamma_hat,
    delta=delta,
    gamma_E=gamma_E,
...
with torch.no_grad():
    # True equilibrium
    prob_a0_true, prob_a1_true = ComputeChoiceProbability(
        s=S_eval,
    ...
    prob_a0_est, prob_a1_est = ComputeChoiceProbability(
        s=S_eval,
