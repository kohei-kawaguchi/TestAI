...
- **Monotonically DECREASING in $s$** (enforced by architecture)
  ...
  - Implemented by negating input: $\hat{P}(s) = \sigma(g(-s))$ where $g$ is monotonic increasing
...
1. $\hat{P}$: Network $\leftarrow$ `InitializeDecreasingCCPNetwork`(hyperparameters) &nbsp;&nbsp;&nbsp;&nbsp;// DECREASING in $s$: wraps monotonic network with input negation $\hat{P}(s) = \sigma(g(-s))$, output $\in [0,1]$
...
- **Monotonic network for CCP**: The CCP network $\hat{P}$ uses monotonic architecture to ensure $\hat{P}(a=1|s)$ is **DECREASING in $s$** (higher capital → lower investment probability). **Implementation**: Negate input before passing to monotonic network: $\hat{P}(s) = \sigma(g(-s))$ where $g$ is monotonic increasing and $\sigma$ is sigmoid to constrain output to $[0,1]$
