...
Estimate the choice probability as a function of state. Since $P(a=1|s)$ is **decreasing** in $s$ (higher capital → lower investment), we instead estimate $P(a=0|s)$ which is **increasing** in $s$:

$$\hat{P}(a=0|s) = \sigma(g_\theta(s))$$
$$\hat{P}(a=1|s) = 1 - \hat{P}(a=0|s)$$

where $g_\theta$ is a neural network that is:
- Continuous in $s$
- **Monotonically INCREASING in $s$** (enforced by architecture)
  - Higher capital stock → Higher probability of not investing
  - **All layers**: weights $w_i \geq 0$ via $\text{softplus}(w_i)$ to ensure monotonic increasing
  - This is the standard monotonic network architecture (all positive weights)
  - Output layer applies sigmoid: $\hat{P}(a=0|s) = \sigma(g_\theta(s))$, output $\in [0,1]$

Then $\hat{P}(a=1|s) = 1 - \hat{P}(a=0|s)$ gives the desired decreasing probability.
...
1. $\hat{P}_0$: Network $\leftarrow$ `EstimateCCP`(states, actions, hyperparameters, num_epochs, learning_rate) &nbsp;&nbsp;&nbsp;&nbsp;// Returns network that predicts $P(a=0|s)$

2. $S$: Tensor[$N \times 1$] $\leftarrow$ `GenerateStateGrid`($N$, state_range)

3. $\hat{P}_{\text{eval}}$: Tensor[$N \times 1$] $\leftarrow 1 - \hat{P}_0(S)$ &nbsp;&nbsp;&nbsp;&nbsp;// Convert to $P(a=1|s) = 1 - P(a=0|s)$ and pre-compute once
...
5. **ParallelFor** $k$: int = 1 to $K$ **do in parallel**: &nbsp;&nbsp;&nbsp;&nbsp;// Evaluate beta candidates in parallel
   ...
   b. distance: float $\leftarrow$ `EvaluateBetaCandidate`($\beta_k$, $\hat{P}_0$, $\hat{P}_{\text{eval}}$, $\hat{\gamma}$, $\delta$, $\gamma_E$, hyperparameters, $S$, max_iter, $\epsilon_{\text{tol}}$, num_epochs, learning_rate)
...
**Procedure** `EvaluateBetaCandidate`($\beta_k$: float, $\hat{P}_0$: Network, $\hat{P}_{\text{eval}}$: Tensor[$N \times 1$], $\gamma$: float, $\delta$: float, $\gamma_E$: float, hyperparameters: dict, $S$: Tensor[$N \times 1$], max_iter: int, $\epsilon_{\text{tol}}$: float, num_epochs: int, learning_rate: float) $\to$ float

1. $v_\theta^{(0)}$, $v_\theta^{(1)}$ $\leftarrow$ `SolveValueFunctionGivenCCP`($\hat{P}_0$, $\beta_k$, $\gamma$, $\delta$, $\gamma_E$, hyperparameters, $S$, max_iter, $\epsilon_{\text{tol}}$, num_epochs, learning_rate) &nbsp;&nbsp;&nbsp;&nbsp;// $\hat{P}_0$ predicts $P(a=0|s)$
...
1. $\hat{P}_0$: Network $\leftarrow$ `InitializeIncreasingCCPNetwork`(hyperparameters) &nbsp;&nbsp;&nbsp;&nbsp;// INCREASING: all layer weights $w_i \geq 0$ via softplus, sigmoid output $\in [0,1]$

2. optimizer: Optimizer $\leftarrow$ `CreateOptimizer`($\hat{P}_0$, learning_rate)
...
3. For epoch: int = 1 to num_epochs:
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$\hat{p}_{0,mt}$: Tensor $\leftarrow \hat{P}_0(s_{mt})$ &nbsp;&nbsp;&nbsp;&nbsp;// Probability of $a=0$ (increasing in $s$)

   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$a_{0,mt}$: int $\leftarrow 1 - a_{mt}$ &nbsp;&nbsp;&nbsp;&nbsp;// Convert to indicator $1\{a=0\}$

   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;loss: Tensor $\leftarrow$ `ComputeBinaryCrossEntropy`($a_{0,mt}$, $\hat{p}_{0,mt}$)

   c. loss.`backward`()

   d. optimizer.`step`()

4. **Return** $\hat{P}_0$ &nbsp;&nbsp;&nbsp;&nbsp;// Returns network that predicts $P(a=0|s)$; compute $P(a=1|s) = 1 - \hat{P}_0(s)$ when needed

---

### Subroutines for Step 2b: Solve Value Function Given CCP

**Procedure** `SolveValueFunctionGivenCCP`($\hat{P}_0$: Network, $\beta$: float, $\gamma$: float, $\delta$: float, $\gamma_E$: float, hyperparameters: dict, $S$: Tensor[$N \times 1$], num_epochs: int, learning_rate: float) $\to$ (Network, Network)

1. $v^{(0)}$: Array[$N$], $v^{(1)}$: Array[$N$] $\leftarrow$ `SolveLinearBellman`($\hat{P}_0$, $\beta$, $\gamma$, $\delta$, $\gamma_E$, $S$) &nbsp;&nbsp;&nbsp;&nbsp;// $\hat{P}_0$ predicts $P(a=0|s)$
...
**Procedure** `SolveLinearBellman`($\hat{P}_0$: Network, $\beta$: float, $\gamma$: float, $\delta$: float, $\gamma_E$: float, $S$: Tensor[$N \times 1$]) $\to$ (Array[$N$], Array[$N$])
...
2. For each action $a$: int $\in \{0, 1\}$:
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;iv. $\hat{p}_{0,i}$: float $\leftarrow \hat{P}_0(s'_i)$ &nbsp;&nbsp;&nbsp;&nbsp;// $P(a=0|s'_i)$ from network

   &nbsp;&nbsp;&nbsp;&nbsp;v. $\hat{p}_i$: float $\leftarrow 1 - \hat{p}_{0,i}$ &nbsp;&nbsp;&nbsp;&nbsp;// Convert to $P(a=1|s'_i) = 1 - P(a=0|s'_i)$

   &nbsp;&nbsp;&nbsp;&nbsp;vi. For $j$: int = 1 to $N$: &nbsp;&nbsp;&nbsp;&nbsp;// Find which grid point $s'_i$ maps to
...
- **Monotonic network for CCP**: We want $P(a=1|s)$ to be **DECREASING in $s$** (higher capital → lower investment probability). Instead of directly predicting this decreasing function, we predict $\hat{P}_0(s) = P(a=0|s)$ which is **INCREASING in $s$**, then compute $P(a=1|s) = 1 - \hat{P}_0(s)$. **Implementation**: Network $\hat{P}_0$ uses standard monotonic increasing architecture with **all layer weights $w_i = \text{softplus}(\tilde{w}_i) \geq 0$** to ensure increasing monotonicity. Output layer applies sigmoid $\sigma$ to constrain to $[0,1]$. Train on complement indicator $1\{a=0\}$ instead of $1\{a=1\}$
- **Nested fixed-point**: The outer loop searches over $\beta$ candidates, inner loop solves value functions for each $\beta$
- **Binary cross-entropy loss**: For CCP estimation, use $-[a_0 \log(\hat{p}_0) + (1-a_0) \log(1-\hat{p}_0)]$ where $a_0 = 1\{a=0\}$ and $\hat{p}_0 = \hat{P}_0(s)$
...
# EstimateCCP returns a network that predicts P(a=0|s)
...
# P_hat_network predicts P(a=0|s), convert to P(a=1|s) = 1 - P(a=0|s)
with torch.no_grad():
    P_hat_a0_eval = P_hat_network(S_eval).numpy().flatten()  # P(a=0|s)
    P_hat_eval = 1 - P_hat_a0_eval  # Convert to P(a=1|s)
