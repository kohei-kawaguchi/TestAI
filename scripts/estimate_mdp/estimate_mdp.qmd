...
- Continuous in $s$
- **Monotonically DECREASING in $s$** (enforced by architecture)
  - Higher capital stock → Lower investment probability
  - **First layer**: weights $w_1 \leq 0$ via $-\text{softplus}(w_1)$ to flip sign
  - **Remaining layers**: weights $w_i \geq 0$ via $\text{softplus}(w_i)$ to preserve monotonicity
  - Composition: positive input → negative (layer 1) → monotonic increasing (layers 2+) → overall decreasing
  - Output layer applies sigmoid: $\hat{P}(s) = \sigma(g(s))$ where $g$ is monotonic decreasing
...
3. $\hat{P}_{\text{eval}}$: Tensor[$N \times 1$] $\leftarrow \hat{P}(S)$ &nbsp;&nbsp;&nbsp;&nbsp;// Pre-compute once
...
2. $P^{\text{updated}}$: Tensor[$N \times 1$] $\leftarrow$ `ComputeCCPFromValue`($S$, $v_\theta^{(0)}$, $v_\theta^{(1)}$)

3. distance: float $\leftarrow$ `ComputeDistance`($\hat{P}_{\text{eval}}$, $P^{\text{updated}}$)
...
1. $\hat{P}$: Network $\leftarrow$ `InitializeDecreasingCCPNetwork`(hyperparameters) &nbsp;&nbsp;&nbsp;&nbsp;// DECREASING: layer 1 weights $w_1 \leq 0$, remaining $w_i \geq 0$, sigmoid output $\in [0,1]$
...
2. For each action $a$: int $\in \{0, 1\}$:
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;iv. $\hat{p}_i$: float $\leftarrow \hat{P}(s'_i)$ &nbsp;&nbsp;&nbsp;&nbsp;// $P(a=1|s'_i)$
...
- **Monotonic network for CCP**: The CCP network $\hat{P}$ uses monotonic architecture to ensure $\hat{P}(a=1|s)$ is **DECREASING in $s$** (higher capital → lower investment probability). **Implementation**: First layer uses negative weights $w_1 = -\text{softplus}(\tilde{w}_1)$ to flip sign. Remaining layers use positive weights $w_i = \text{softplus}(\tilde{w}_i)$ to preserve monotonicity. This achieves: positive input $s$ → negative (layer 1) → monotonic increasing through remaining layers → overall decreasing. Output layer applies sigmoid $\sigma$ to constrain to $[0,1]$
