...
- **Monotonically DECREASING in $s$** (enforced by architecture)
  ...
  - Implemented by constraining all weights to be **non-positive** ($w_{ij} \leq 0$)
  - Uses negative softplus: $-\text{softplus}(w)$ to ensure $w \leq 0$
  - Monotonic decreasing activation preserves monotonicity through layers
...
1. $\hat{P}$: Network $\leftarrow$ `InitializeDecreasingCCPNetwork`(hyperparameters) &nbsp;&nbsp;&nbsp;&nbsp;// DECREASING: all weights $w \leq 0$ via $-\text{softplus}(w)$, sigmoid output $\in [0,1]$
...
- **Monotonic network for CCP**: The CCP network $\hat{P}$ uses monotonic architecture to ensure $\hat{P}(a=1|s)$ is **DECREASING in $s$** (higher capital → lower investment probability). **Implementation**: Constrain all weights to be non-positive using $w = -\text{softplus}(\tilde{w})$ where $\tilde{w}$ are unconstrained parameters. This ensures monotonic decrease: positive input $s$ → negative pre-activation → monotonic decrease through layers. Output layer applies sigmoid $\sigma$ to constrain to $[0,1]$
