...
- config: dict
  ...
  - data: dict (`Z`, `S_X`, `S_W`)
  - solver: dict (`max_iter`, `tol`, `step_size`, `damping`)
...
3. `p_current` <- config["initialization"]["p_init"]
4. For `iteration` in `0..config["solver"]["max_iter"]-1`:
   - `residual` <- `ComputeFOCResidual`(p=p_current, config=config)
   - `residual_norm` <- `ComputeResidualNorm`(residual=residual)
   - If `residual_norm < config["solver"]["tol"]`, return `(p_current, diagnostics)`
   - `p_next` <- `UpdatePriceVector`(
       p=p_current,
       residual=residual,
   ...
   - `p_current` <- p_next
5. Return `(p_current, diagnostics)`
...
5. Return `(s_inside, s_outside)`

Procedure `ComputeShareJacobian`(s_inside: Vector[J], alpha: float) -> Matrix[JxJ]

1. For each `(j, k)` in `{1, ..., J} x {1, ..., J}`:
...
3. `jacobian` <- `ComputeShareJacobian`(s_inside=s_inside, alpha=config["model"]["alpha"])
...
Procedure `UpdatePriceVector`(p: Vector[J], residual: Vector[J], step_size: float, damping: float) -> Vector[J]

1. `direction` <- `-step_size * residual`
2. Return `p + damping * direction`
