...
- diagnostics: dict (`residual_norm`, `iterations`, `converged`, `solver_status`, `residual_history`)
...
4. Return `(exp_delta, denom, s_inside)`

Procedure `ComputeShares`(delta: Vector[J]) -> (Vector[J], float)

1. `(_, denom, s_inside)` <- `ComputeShareCoreTerms`(delta=delta)
2. `s_outside` <- `1.0 / denom`
3. Return `(s_inside, s_outside)`

Procedure `ComputeShareJacobian`(delta: Vector[J], alpha: float) -> Matrix[JxJ]

1. `(_, _, s_inside)` <- `ComputeShareCoreTerms`(delta=delta)
2. For each `(j, k)` in `{1, ..., J} x {1, ..., J}`:
...
2. `(s_inside, _)` <- `ComputeShares`(delta=delta)
3. `jacobian` <- `ComputeShareJacobian`(delta=delta, alpha=config["model"]["alpha"])
...
1. Return dict with keys: `residual_norm`, `iterations`, `converged`, `solver_status`, `residual_history`
...
1. Define three experiments:
   - vary `alpha` over `statics["alpha_values"]`
   - vary `beta[0]` over `statics["beta_0_values"]`
   - vary `gamma[1]` over `statics["gamma_1_values"]`
2. For each experiment and each grid value:
   - copy `base_config`
   - update only the target parameter
   - set initialization price to the previous solved equilibrium (warm start)
   - solve equilibrium with `SolveNashEquilibrium`
   - compute endogenous outcomes (`mean_price`, `mean_markup`, `mean_inside_share`)
3. Stack all outcomes into one table and return it.
...
print(f"  validation_threshold: {validation_tol:.3e}")
```
...
The comparative statics follow the economic structure of the logit-Bertrand model. When the price coefficient increases, consumers become more price sensitive, so firms optimally set lower prices and accept lower markups. The inside shares also decline because higher price sensitivity amplifies the demand loss from any positive markup relative to marginal cost.

When the first demand coefficient increases, products become more attractive at a given price, which raises equilibrium prices and markups. The mean inside share also rises because the utility index shifts upward, moving demand from the outside option toward inside goods. The monotone increase across the selected grid indicates stable comparative-statics behavior in this specification.

When the second cost coefficient increases, marginal costs rise through the cost index, and equilibrium prices increase accordingly. Markups decline modestly over this range, which is consistent with incomplete pass-through under downward-sloping demand. At the same time, inside shares fall as higher costs feed into higher prices and reduce demand. Across all three experiments, solutions remain converged with small residual norms, which supports the numerical reliability of these behavioral comparisons.
