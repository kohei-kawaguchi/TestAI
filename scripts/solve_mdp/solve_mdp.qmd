...
**Procedure** `SolveValueIteration`($\beta$, $\gamma$, $\delta$, hyperparameters)

1. $v_\theta^{(0)}, v_\theta^{(1)} \leftarrow$ `InitializeNetworks`(hyperparameters)

2. $S \leftarrow$ `GenerateStateGrid`($N$, state_range)

3. For `iteration` = 1 to `max_iter`:

   a. $\{y_i^{(a)}\} \leftarrow$ `ComputeBellmanTargets`($S$, $v_\theta^{(0)}$, $v_\theta^{(1)}$, $\beta$, $\gamma$, $\delta$)

   b. $v_\theta^{(0)}, v_\theta^{(1)} \leftarrow$ `UpdateNetworks`($S$, $\{y_i^{(a)}\}$, $v_\theta^{(0)}$, $v_\theta^{(1)}$, num_epochs)

   c. max_error $\leftarrow$ `CheckConvergence`($S$, $\{y_i^{(a)}\}$, $v_\theta^{(0)}$, $v_\theta^{(1)}$)
...
**Procedure** `InitializeNetworks`(hyperparameters)

- Create networks $v_\theta^{(0)}$ and $v_\theta^{(1)}$ with monotonic weight constraints
- Initialize parameters $\theta$ randomly
- **Return** $v_\theta^{(0)}$, $v_\theta^{(1)}$

---

**Procedure** `GenerateStateGrid`($N$, state_range)

- Create uniform grid: $S = \{s_1, \ldots, s_N\}$ over state_range
- **Return** $S$

---

**Procedure** `ComputeBellmanTargets`($S$, $v_\theta^{(0)}$, $v_\theta^{(1)}$, $\beta$, $\gamma$, $\delta$)

For each $s_i \in S$ and $a \in \{0, 1\}$:

&nbsp;&nbsp;&nbsp;&nbsp;$s'_i \leftarrow$ `ComputeNextState`($s_i$, $a$, $\gamma$)

&nbsp;&nbsp;&nbsp;&nbsp;$\text{EV}_i \leftarrow$ `ComputeExpectedValue`($s'_i$, $v_\theta^{(0)}$, $v_\theta^{(1)}$, $\gamma_E$)

&nbsp;&nbsp;&nbsp;&nbsp;$y_i^{(a)} \leftarrow$ `ComputeMeanReward`($s_i$, $a$, $\beta$) $+ \delta \cdot \text{EV}_i$

**Return** $\{y_i^{(a)}\}$ for all $i$, $a$

---

**Procedure** `ComputeNextState`($s$, $a$, $\gamma$)

- **Return** $(1 - \gamma) \cdot s + a$

---

**Procedure** `ComputeMeanReward`($s$, $a$, $\beta$)

- **Return** $\beta \cdot s - a$

---

**Procedure** `ComputeExpectedValue`($s'$, $v_\theta^{(0)}$, $v_\theta^{(1)}$, $\gamma_E$)

- $v_0 \leftarrow v_\theta^{(0)}(s')$
- $v_1 \leftarrow v_\theta^{(1)}(s')$
- $\text{EV} \leftarrow$ `LogSumExp`($v_0$, $v_1$) $+ \gamma_E$
- **Return** EV

---

**Procedure** `LogSumExp`($v_0$, $v_1$)

- $\text{max}_v \leftarrow \max(v_0, v_1)$
- **Return** $\text{max}_v + \log(\exp(v_0 - \text{max}_v) + \exp(v_1 - \text{max}_v))$

---

**Procedure** `UpdateNetworks`($S$, $\{y_i^{(a)}\}$, $v_\theta^{(0)}$, $v_\theta^{(1)}$, num_epochs)

For `epoch` = 1 to num_epochs:

&nbsp;&nbsp;&nbsp;&nbsp;$L \leftarrow$ `ComputeLoss`($S$, $\{y_i^{(a)}\}$, $v_\theta^{(0)}$, $v_\theta^{(1)}$)

&nbsp;&nbsp;&nbsp;&nbsp;$\nabla_\theta L \leftarrow$ `ComputeGradients`($L$)

&nbsp;&nbsp;&nbsp;&nbsp;$\theta \leftarrow \theta - \text{learning\_rate} \cdot \nabla_\theta L$

**Return** $v_\theta^{(0)}$, $v_\theta^{(1)}$

---

**Procedure** `ComputeLoss`($S$, $\{y_i^{(a)}\}$, $v_\theta^{(0)}$, $v_\theta^{(1)}$)

- $L \leftarrow \sum_{i=1}^N \sum_{a \in \{0,1\}} (v_\theta^{(a)}(s_i) - y_i^{(a)})^2$
- **Return** $L$

---

**Procedure** `CheckConvergence`($S$, $\{y_i^{(a)}\}$, $v_\theta^{(0)}$, $v_\theta^{(1)}$)

- max_error $\leftarrow \max_{i,a} |v_\theta^{(a)}(s_i) - y_i^{(a)}|$
- **Return** max_error
