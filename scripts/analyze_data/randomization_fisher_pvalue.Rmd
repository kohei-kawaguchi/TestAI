---
title: "Neyman's Average Treatment Effect"
...
1. Treatment assignment is not independent in finite samples—in a completely randomized experiment, knowing some assignments provides information about others
...
## Summary of Estimation and Inference Results

```{r summary-table}
# Create summary table
results_summary <- data.frame(
  Statistic = c("Total Sample Size", "Treated Sample Size", "Control Sample Size",
                "Mean Outcome (Treated)", "Mean Outcome (Control)",
                "Average Treatment Effect", "Standard Error",
                "95% CI Lower Bound", "95% CI Upper Bound",
                "t-statistic", "p-value (two-sided)"),
  Value = c(
    nrow(data),
    n_treated,
    n_control,
    ...
    ate + 1.96 * se_ate,
    ate / se_ate,
    2 * (1 - pnorm(abs(ate / se_ate)))
  )
)

kable(results_summary, digits = 3, caption = "Summary of Estimation and Inference Results")
```
...
plot_data <- data.frame(
  ...
  Mean = c(mean_control, mean_treated),
  SE = c(sqrt(var_control / n_control), sqrt(var_treated / n_treated))
)
...
ggplot(plot_data, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(
    title = "Mean Outcomes by Treatment Group",
